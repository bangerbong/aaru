...
function setupDay1() {
    ...
    hl.innerHTML = ""; ol.innerHTML = ""; // Reset
    for(let i=0; i<5; i++) {
...
function setupDay2() {
    ...
    document.getElementById('snoopy-house').style.left = "20px";
...
document.querySelectorAll('.lane').forEach(lane => {
    lane.onclick = () => {
        if(isPlacing && hearts >= 50) {
            ...
            w.src = "Woodstock.png"; w.className = "defender"; w.style.left = "20px";
            lane.appendChild(w);
            hearts -= 50;
...
function shootHeart(lane) {
    ...
    b.innerHTML = "❤️"; b.className = "bullet"; b.style.left = "70px"; b.style.top = "30%";
    lane.appendChild(b);
    let bX = 70;
    const move = setInterval(() => {
        bX += 5; b.style.left = bX + "px";
        const enemy = lane.querySelector('.enemy');
        if(enemy && bX > enemy.offsetLeft - 20) {
            enemy.remove(); b.remove(); clearInterval(move);
        }
...
function spawnDistraction() {
    ...
    en.innerHTML = "📝"; en.className = "enemy"; en.style.left = "1000px";
    lane.appendChild(en);
    let eX = 1000;
    const walk = setInterval(() => {
        if(!gameActive) { clearInterval(walk); return; }
        eX -= 2; en.style.left = eX + "px";
        if(eX < 60) { clearInterval(walk); alert("Snoopy got distracted! Try again ❤️"); location.reload(); }
    }, 30);
}
...
function showWin() {
    ...
    document.getElementById('final-letter').onclick = () => { 
        alert("Day 1 Complete! I love you so much. ❤️"); 
        location.reload(); 
    };
}
